...
const string strMessageMagic = "CryptoBoxxy Signed Message:\n";
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
	...
	if(nHeight < 1049000){
		nSubsidy = (1050000 - nHeight) * COIN;
}

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 4 * 60 * 60; // CryptoBoxxy: 1 days
static const int64 nTargetSpacing = 60; // CryptoBoxxy: 1 minute blocks
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal CryptoBoxxy miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "CryptoBoxxy", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "February 3rd 2014 GOOG.O on Nasdaq 1,137.35USD";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1391178908; //epochtime
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1345972;

        if (fTestNet)
        {
            block.nTime    = 1391178908;
            block.nNonce   = 440824;
        }
